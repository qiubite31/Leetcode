...
class Solution(object):
    def subdomainVisits(self, cpdomains):
        ...
        :rtype: List[str]
        """
        all_visit_times = dict()
        for cpdomain in cpdomains:
            visit_times = int(cpdomain.split(' ')[0])
            domain = cpdomain.split(' ')[1]
            all_visit_times[domain] = all_visit_times.setdefault(domain, 0) + visit_times

            sub_domain = domain
            while '.' in sub_domain:
                sub_domain = sub_domain.split('.', 1)[1]
                all_visit_times[sub_domain] = all_visit_times.setdefault(sub_domain, 0) + visit_times
