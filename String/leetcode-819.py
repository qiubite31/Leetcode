...
class Solution(object):
    def mostCommonWord(self, paragraph, banned):
        ...
        :rtype: str
        """
        banned_dict = {word: 1 for word in banned}
        tf = {}
        for word in paragraph.lower().split(' '):
            ...
            word = word.replace("'", '')
            try:
                if banned_dict[word] == 1:
                    continue
            except KeyError as err:
                tf[word] = tf.setdefault(word, 0) + 1

        return sorted(tf.items(), key=lambda x: x[1], reverse=True)[0][0]
    ...
    def mostCommonWord2(self, paragraph, banned):
        ...
        # print(Counter(word for word in words if word not in banned).most_common(1)[0][0])
...
solution = Solution()
output = solution.mostCommonWord2(paragraph, banned)
